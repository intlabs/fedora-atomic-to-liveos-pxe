...
guestmount -a $IMAGE -m /dev/sda1 $BOOT_TARGET
guestmount -a $IMAGE -m /dev/atomicos/root $ROOT_TARGET
...
cat $FSTAB_ORIG | grep -v boot > $FSTAB_CONVERT
cp $FSTAB_CONVERT $FSTAB_ORIG

echo "Done removing boot from /etc/fstab."

echo "Extracting kernel to ${IMAGE_TARGET}-kernel"
cp $BOOT_IMAGES_BASE/vmlinuz-3.17.4-301.fc21.x86_64 $IMAGE_TARGET-kernel

echo "Extracting ramdisk to ${IMAGE_TARGET}-ramdisk"
cp $BOOT_IMAGES_BASE/initramfs-3.17.4-301.fc21.x86_64.img $IMAGE_TARGET-ramdisk
...
qemu-img convert $IMAGE ${IMAGE_TARGET}.raw
